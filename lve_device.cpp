...
namespace lve
{
    ...
    void LveDevice::createInstance()
    {
        ...
        // START: Setup required in MAC systems to fix instance creation failure error

        std::vector<const char *> requriedExtensions;

        for (uint32_t i = 0; i < createInfo.enabledExtensionCount; i++)
        {
            requriedExtensions.emplace_back(extensions[i]);
        }

        requriedExtensions.emplace_back(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME);
        requriedExtensions.emplace_back(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);

        createInfo.flags |= VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR;

        createInfo.enabledExtensionCount = (u_int32_t)requriedExtensions.size();

        createInfo.ppEnabledExtensionNames = requriedExtensions.data();

        // END: Setup required in MAC systems to fix instance creation failure error
